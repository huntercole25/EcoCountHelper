...
Without providing too much detail, the QQ-plot (the furthest left plot) should show a line of points that are relatively straight, and that fall close to the red line running diagonally across the plot. The dispersion plot (second from left) should show a histogram with a peak close to the vertical red line shown in the plot. The residual outlier distribution (third from left) should show a relatively flat histogram with few or no bars filled with red. The histogram of residual outliers resulting from bootstrapped residual simulations (furthest right) should have the expected outlier frequency (black vertical line) near the actual outlier frequency (red vertical line). For more information on what these plots mean, please see [this DHARMa vignette](https://cran.r-project.org/web/packages/DHARMa/vignettes/DHARMa.html). Additionally, it is important to note that, because these plots are based on simulations, no two function calls will result in identical plots, although the results should be very similar and lead to the same conclusion regarding goodness-of-fit.
...
```{r AlphaSp}
setcolorder(x = RealEfWide, neworder = c("Predictor", "UnitChange",
                                         "Epfu", "Laci", "Lano", 
                                         "Myev", "Mylu", "Myvo",
                                         "Myyu"))
```
...
The last step we need to take before creating a formatted table is to record the rows that fall under each predictor classification (anthropogenic or natural) and delete the "PredNature" vector. From the table above, we know that anthropogenic predictors occupy rows one through four, and natural predictors occupy rows five through nine. With that information recorded, we can remove the "PredNature" vector from the "RealEfWide" table.  
